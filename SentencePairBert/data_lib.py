...
class XnliProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        for (i, (e, z, l)) in enumerate(zip(en, zh, label)):
            if i == 0:
                continue
            guid = "train-%d" % i
            text_a = self.process_text_fn(e)
            text_b = self.process_text_fn(z)
            label = self.process_text_fn(l)
            examples.append(
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        en = self._read(os.path.join(data_dir, "en.eval1"))
        zh = self._read(os.path.join(data_dir, "zh.eval1"))
        label = self._read(os.path.join(data_dir, "label.eval1"))
        examples = []
        for (i, (e, z, l)) in enumerate(zip(en, zh, label)):
            if i == 0:
                continue
