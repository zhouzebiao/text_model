...
def generate_synthetic_data(
        source_shape, source_value=0, source_dtype=None,
        target_shape=None, target_value=0, target_dtype=None,
        label_shape=None,
    ...
    source_element = nest.map_structure(
        lambda s: tf.constant(source_value, source_dtype, s), source_shape)
    target_element = nest.map_structure(
        lambda s: tf.constant(target_value, target_dtype, s), target_shape)
    element = (source_element, target_element)
    if label_shape:
        label_element = nest.map_structure(
            lambda s: tf.constant(label_value, label_dtype, s), label_shape)
        element = (source_element, target_element, label_element)
