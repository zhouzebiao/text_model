# -*- coding: utf-8 -*-
"""
 Created by zaber on 2019-12-09 14:
"""
import numpy as np
...
class SequenceBeamSearchV2():
    ...
    def __init__(self, symbols_to_logits_fn, vocab_size, batch_size,
                 beam_size, alpha, max_decode_length, eos_id, data_type):
    ...
    def _create_initial_state(self, initial_ids, initial_cache):
        ...
        initial_log_probs = tf.constant(
            [[0.] + [-float("inf")] * (self.beam_size - 1)], dtype=self.data_type)
        ...
        finished_scores = tf.ones([self.batch_size, self.beam_size],
                                  dtype=self.data_type) * -inf(self.data_type)
    ...
    def _continue_search(self, state):
        ...
        finished_scores *= tf.cast(finished_flags,
                                   self.data_type)  # set filler scores to zero
        ...
        lowest_finished_scores += ((1.0 -
                                    tf.cast(finished_batches, self.data_type)) *
                                   -inf(self.data_type))
    ...
    def _get_new_alive_state(self, new_seq, new_log_probs, new_cache):
        ...
        new_log_probs += tf.cast(new_finished_flags, self.data_type) * -inf(self.data_type)
    ...
    def _get_new_finished_state(self, state, new_seq, new_log_probs):
        ...
        new_finished_flags = tf.equal(new_seq[:, :, -1], self.eos_id)
        new_scores += ((1. - tf.cast(new_finished_flags, self.data_type)) *
                       -inf(self.data_type))
